...
#define FLEXCAN_IP_HWACCESS_SW_MAJOR_VERSION_C               1
...
#define FLEXCAN_IP_RX_FIFO_FILTER_TABLE_OFFSET      0x18U
...
#define FLEXCAN_IP_ENHANCED_RX_FIFO_ONE_ID_FILTER    0UL
#define FLEXCAN_IP_ENHANCED_RX_FIFO_RANGER_ID_FILTER 1UL
#define FLEXCAN_IP_ENHANCED_RX_FIFO_TWO_ID_FILTER    2UL
...
Flexcan_Ip_StatusType FlexCAN_SetMaxMsgBuffNum(FLEXCAN_Type * base, uint32 maxMsgBuffNum)
{
    ...
    {
        ValEndRam = (uint32)&RAM[FLEXCAN_IP_RAMn_COUNT];
    }
#else
    ValEndRam = (uint32)&RAM[FLEXCAN_IP_RAMn_COUNT];
#endif /* FLEXCAN_IP_FEATURE_HAS_EXPANDABLE_MEMORY */
                            ...
                            )
{
    ...
    volatile uint32 * FilterTable = (uint32 *)((uint32)base + (uint32)FLEXCAN_IP_FEATURE_RAM_OFFSET + ((uint32)FLEXCAN_IP_RX_FIFO_FILTER_TABLE_OFFSET * 4U));
...
void FlexCAN_SetEnhancedRxFifoFilter(FLEXCAN_Type * base, const Flexcan_Ip_EnhancedIdTableType * idFilterTable)
{
    ...
    FilterExtIDTable = &base->ERFFEL[FLEXCAN_IP_ENHANCED_RX_FIFO_FILTER_TABLE_BASE];
    ...
    for (i = 0U; i < (NumOfExtIDFilter + NumOfStdIDFilter); i++)
    {
        if (!(idFilterTable[i].isExtendedFrame))
        {
            ...
            switch (idFilterTable[i].filterType)
            {
                ...
                {
                    FilterStdIDTable[j] |= FLEXCAN_IP_ENHANCED_RX_FIFO_ONE_ID_FILTER << FLEXCAN_IP_ENHANCED_RX_FIFO_ID_FILTER_FSCH_SHIFT;
                }break;

                case FLEXCAN_ENHANCED_RX_FIFO_RANGE_ID_FILTER :
                {
                    FilterStdIDTable[j] |= FLEXCAN_IP_ENHANCED_RX_FIFO_RANGER_ID_FILTER << FLEXCAN_IP_ENHANCED_RX_FIFO_ID_FILTER_FSCH_SHIFT;
                }break;

                case FLEXCAN_IP_ENHANCED_RX_FIFO_TWO_ID_FILTER :
                {
                    FilterStdIDTable[j] |= FLEXCAN_IP_ENHANCED_RX_FIFO_TWO_ID_FILTER << FLEXCAN_IP_ENHANCED_RX_FIFO_ID_FILTER_FSCH_SHIFT;
                }break;
                default:
                {
#if (FLEXCAN_IP_DEV_ERROR_DETECT == STD_ON)
        ...
        else
        {
            ...
            switch (idFilterTable[i].filterType)
            {
                case FLEXCAN_IP_ENHANCED_RX_FIFO_ONE_ID_FILTER :
                {
                    FilterExtIDTable[n] |= FLEXCAN_IP_ENHANCED_RX_FIFO_ONE_ID_FILTER << FLEXCAN_IP_ENHANCED_RX_FIFO_ID_FILTER_FSCH_SHIFT;
                }break;
                case FLEXCAN_ENHANCED_RX_FIFO_RANGE_ID_FILTER :
                {
                    FilterExtIDTable[n] |= FLEXCAN_IP_ENHANCED_RX_FIFO_RANGER_ID_FILTER << FLEXCAN_IP_ENHANCED_RX_FIFO_ID_FILTER_FSCH_SHIFT;
                }break;
                case FLEXCAN_IP_ENHANCED_RX_FIFO_TWO_ID_FILTER :
                {
                    FilterExtIDTable[n] |= FLEXCAN_IP_ENHANCED_RX_FIFO_TWO_ID_FILTER << FLEXCAN_IP_ENHANCED_RX_FIFO_ID_FILTER_FSCH_SHIFT;
                }break;
                default :
                {
#if (FLEXCAN_IP_DEV_ERROR_DETECT == STD_ON)
